...
var stickyTabHeaderMinHeight = 135;
			...
			// update a hash into current location
				...
				if (mediaQuery === media_xs) {
					...
					if (idForHash !== null) {
						...
						// hotfix(v1.2: 20140515): if browser is a Chrome in iPhone, use history because location.hash doesn't be saved properly. so need extra history saving.
...
//			alert('it is not normal anchor tag');
		}
...
/*
function moveToPagePosition2() {

	var target = window.location.hash;
	var realTarget = target.replace('ING-', '');

	if (target.length > 1 && $(realTarget).size() === 1) {
...
//				}, 500);
				...
				$('[data-target="' + realTarget + '"]').each(function(index, el) {
					if ($(el).hasClass('mobile-tab') === true) {
						$(el).addClass('active');
						//console.log('[id="' + realTarget + '"]');
						$('[id="' + realTarget.replace('#', '') + '"]').css({
							'display': 'block'
						});
						$('body, html').stop().animate({scrollTop : $(el).offset().top}, 10);
...
//	var contentTopHeight = headerTopHeight + $('.ING-tab-header').prev('div, section, article').height();
	...
	var contentTopHeight = headerTopHeight + infoHeight + heroHeight;
...
//		console.log(stickyTabIds[index]);
	...
	var barWidth = (1/tabListCount)*100.toFixed(4);
	$('<div/>')
		.addClass('orange-bar')
		.addClass('invisible')
		.css('width', barWidth + '%')
